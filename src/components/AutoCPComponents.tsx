...
import { findBest, solve_paadec, solve_tau } from "../libs/calculations_extended.ts";
...
export default function AutoCPComponents({ width, height, pdc, initialParams, verbose = false, margin = defaultMargin }: AutoCPComponentProps) {
  ...
  if (verbose) console.log(pdc);

  //get sample points for optimization
  const mmpData:MMPDataPoint[] = useMemo(() => {

    const tempData:MMPDataPoint[] = [];
    const currentPDC: StrydPDC = {...pdc};

    targetTimePoints.map((point, i) => {
      //console.log(point, pdc.breakdown.total[point-1])
      if (currentPDC.breakdown.total[point-1] != undefined) {
        tempData[i] = {
          time:point,
          power: currentPDC.breakdown.total[point-1],
          c1: currentPDC.breakdown.alactic[point-1],
          c2: currentPDC.breakdown.anaerobic[point-1],
          c3: currentPDC.breakdown.aerobic[point-1],
        }
      }
    })
    return tempData
  ...
  const calculatedParams = useMemo(() => {
    ...
    if (verbose) console.log("confirm initial params", params.cp, params.tau, params.paadec, params.paa);
    ...
    const bestC3 = findBest(60*9, 60*60, currentPDC.breakdown.aerobic)
    if (verbose) console.log("bestC3", bestC3);
    params.cp = bestC3.power;

    let bestTime;

    bestTime = 3 * 60;
    const best3min = currentPDC.breakdown.total[bestTime-1];
    params.tau = solve_tau(bestTime, best3min, { ...params });
    if (verbose) console.log("best 3 minute", bestTime, best3min, params.cp, params.tau)

    bestTime = 5 * 60;
    const best5min = currentPDC.breakdown.total[bestTime-1];
    params.tau = solve_tau(bestTime, best5min, { ...params });
    if (verbose) console.log("best 5 minute", bestTime, best5min, params.cp, params.tau)

    bestTime = 60;
    const best1min = currentPDC.breakdown.total[60-1];
    if (verbose) console.log("best1min", best1min);
    params.paadec = solve_paadec(bestTime, best1min, { ...params });
    if (verbose) console.log("paadec from GC", params.paadec);
  ...
  const optimizationSolution = useMemo(() => {
    ...
    if (verbose) console.log(optimizationGD)
    console.log(
