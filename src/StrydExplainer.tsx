      ...
      <div className="mt-2 flex grow flex-col gap-2 md:flex-row">
        ...
        <div className="flex gap-2 flex-col grow rounded-lg bg-gray-50 px-2 py-2">
          ...
          <ul className="list-disc list-inside dark:text-gray-400">
            <li><span className="font-bold">What Stryd Things your Extended CP model looks like</span> It takes the extended model curves which are on - but not yet displayed - the Stryd website and displays them and uses multivariable regression to try to determine what Stryd things your Extended model variables are</li>
            <li><span className="font-bold">What this app your Extended CP model looks like</span> It takes the max power data from the Stryd website and tries to "recreate" AutoCP values using an interpretation of the Stryd "Special Sauce" / GC algorithm</li>
            <li><span className="font-bold">"What if" scenarios</span> It allows you to adjust the max power data at different time points for various "what if" scenarios</li>
          </ul>
...
`{
  ...
  "breakdown": {
    ...
    "total": ["alactic + anaerobic + aerobic + "fudge factor" to power at 1s is still max power"]
  }
}`}
          </code></pre>
          ...
          <ul>
            ...
            <li><span className="font-bold">iterations:</span> how many iterations were used to estimate the Stryd Extended CP model parameters. Opinions vary on what these should be. I include 22 as a max because Stryders have this data - not because the literature says it should be valid.</li>
            <li><span className="font-bold">CP "valid" region:</span> the time range used to sample points to do a comparison linear CP estimate using "X" points (not everyone has 22 min runs)</li>
          ...
          <p>For example, the following curve has excellent fit (RSME are all very low). However, tau is close to 0.5 and shaded red to flag it.</p>
          ...
          <p>If I increase the value of 180s from 342 W to 365 W nothing will happen as 376W at 131s is still the max power that determines the final tau in this time box. However, if I use 180s @ 366 W then there is an update. Tau has updated so W' (which is based on tau) has increased. Therefore, the CP calulation must decrease as max power has a greater percentage coming from anaerobic power - and there less from aerobic power. "Classic" teeter-tooter effect!</p>
          ...
          <p>Here is another scenario. Imagine I have a CP test planned. I have been told that I need to run an 8min (480s) test. I can input time 4 as 480s and then increase the power value to see the result. Spoiler: there is no updated. Why? My testing shows that adding power files with less than 9min (540s) duration do not update the calculations.
            In fact, if I upload ONLY two files, a 3 min and an 8 min test, Styrd returns a PDC.json that does not have the anaerobic or aerobic curve information (Uploading ONLY a 3 min and 9 minute works fine). For this reason, I recommend always testing at least 9 min. On this same note, the original GC calculations had 6 min in the code. Why it seems like Stryd uses 9 min is not clear, but I really think this is the case.
            On this same note, GC uses a high point of this test at 1800s (30 min). I have not found any reason in my testing not to thing this is also the Stryd value.</p>
          ...
          <p>I uploaded two power files to Styrd (only). One with 180s @ 350 W and one with 540s @ 400 W. The paadec was -3. I uploaded a third file with 540s @ 300 W and the paadec was -3.<br/>
